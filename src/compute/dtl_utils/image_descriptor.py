...
class ImageDescriptor:
    def __init__(self, info, modify_ds_name=True):
    ...
    def write_image_local(self, img_path):
        if self.image_data is None:
            raise RuntimeError(
                "ImageDescriptor [write_image_local] image_data is None:{}".format(img_path)
            )
    ...
    def set_img_name(self, new_name):
        self.info.image_name = new_name

    def clone_with_name(self, new_name):
        self.info: NamedTuple
        new_info = self.info._replace(image_name=new_name)
        new_obj = self.__class__(new_info)
        new_obj.image_data = self.image_data
        new_obj.res_ds_name = self.res_ds_name
        return new_obj
