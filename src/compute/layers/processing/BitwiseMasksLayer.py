...
class BitwiseMasksLayer(Layer):
    ...
    def process(self, data_el):
        ...
        if mask_label is not None:
            ...
            full_target_mask[target_origin.row:target_origin.row + target_mask.shape[0],
                             target_origin.col:target_origin.col + target_mask.shape[1]] = target_mask

            new_labels = []

            for label in ann.labels:
                if label.obj_class.name not in self.settings['classes_to_correct'] or label.obj_class.name == class_mask_name:
